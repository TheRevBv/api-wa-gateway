...
const environmentSchema = z
  .object({
    ...
    GATEWAY_PUBLIC_API_BEARER_TOKEN: z.string().default(""),
  ...
  .superRefine((env, ctx) => {
    ...
    if (env.GATEWAY_PUBLIC_API_BEARER_TOKEN.trim().length === 0) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["GATEWAY_PUBLIC_API_BEARER_TOKEN"],
        message: "GATEWAY_PUBLIC_API_BEARER_TOKEN is required in production"
      });
    }
  });
