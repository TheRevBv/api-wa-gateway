...
export const registerConversationRoutes = (
  ...
  app.get("/api/v1/tenants/:tenantId/conversations", async (request) => {
    ...
    const result = await dependencies.listConversations.execute(params.tenantId, query);
  ...
  app.get("/api/v1/tenants/:tenantId/conversations/:conversationId", async (request) => {
    const params = conversationParamsSchema.parse(request.params);
    const result = await dependencies.getConversation.execute(params.tenantId, params.conversationId);
  ...
  app.get("/api/v1/tenants/:tenantId/conversations/:conversationId/messages", async (request) => {
    const params = conversationParamsSchema.parse(request.params);
    const query = paginationSchema.parse(request.query);
